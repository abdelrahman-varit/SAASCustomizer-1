...
$factory->define(\Webkul\Product\Models\ProductFlat::Class, function (Faker $faker, $data) {
    ...
    if ($products->type == 'simple') {
        ...
        $fakeData = app('Webkul\SAASCustomizer\Repositories\ProductFlatRepository')->GetProductFlatDummyData($faker, $products->type);
        ...
        $fakeImage = app('Webkul\SAASCustomizer\Repositories\ProductFlatRepository')->uploadImages($faker, $product);

        factory(\Webkul\Product\Models\ProductImage::class, 5)->create($fakeImage);


        $category = app('Webkul\SAASCustomizer\Repositories\ProductFlatRepository')->createProductCategories($product, $faker);
