@php
    ...
    $attributeOptionTranslations = app('\Webkul\Attribute\Repositories\AttributeOptionRepository')->with('translations')->whereHas('translations', function ($query) use ($locale) {
        $query->where('locale', $locale);
    })->get();
    
    $arrtibuteOptionTrans = [];
    if ( $attributeOptionTranslations ) {
        foreach($attributeOptionTranslations as $attributeOptionTranslation) {
            $arrtibuteOptionTrans[] = $attributeOptionTranslation->translations->first();
        }
    }

    $arrtibuteOptionTrans = new \Illuminate\Database\Eloquent\Collection($arrtibuteOptionTrans);
    $arrtibuteOptionTrans = $arrtibuteOptionTrans->toJson();
@endphp
        ...
        Vue.component('compare-product', {
            ...
            data: function () {
                return {
                    ...
                    'attributeOptions': JSON.parse(@json($arrtibuteOptionTrans)),
